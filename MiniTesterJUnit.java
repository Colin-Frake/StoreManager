...
public class MiniTesterJUnit {
    ...
    @BeforeEach
    @Tag("score:1")
    @DisplayName("Set MAX_PACK_SIZE and BUZZY_HAPPINESS_BOOST to 6 and 1 respectively and other static classes")
    void setUpStaticVariables() throws IllegalAccessException {
        try {
            ...
            fail("MAX_PACK_SIZE and BUZZY_HAPPINESS_BOOST should be declared in Drink class");
        }
    ...
    void testAbstractClasses() {
        ...
        assertTrue(Modifier.isAbstract(StoreItem.class.getModifiers()), "HoneyBee should be an abstract class..");
        assertTrue(Modifier.isAbstract(Drink.class.getModifiers()), "Insect should be an abstract class..");
    }
    ...
    private void addItemReflection(ItemList itemList, StoreItem item) throws IllegalAccessException, NoSuchFieldException {
        // Access the 'items' and 'size' fields via reflection
        Field itemsField = ItemList.class.getDeclaredField("items");
        // Make the field accessible
        itemsField.setAccessible(true);
        // Get the current array
        StoreItem[] items = (StoreItem[]) itemsField.get(itemList);

        // use the getSize method to get size
        int size = itemList.getSize();

        // Resize logic if necessary (double the array size)
        if (size >= items.length) {
            StoreItem[] larger = new StoreItem[items.length * 2];
            System.arraycopy(items, 0, larger, 0, items.length);
            itemsField.set(itemList, larger);  // Set the resized array back
            items = larger;
        }
        ...
        // Update the 'size' field with the new size
    }
    ...
    // Check that the BUZZY_HAPPINESS_BOOST is applied when this.buzziness is true.
    @Test
    @Tag("score:1")
    @Order(33)
    ...
    // combine Check that it returns false when bottles are NOT equal
    ...
    private StoreItem[]  getStoreItems(ItemList itemList) throws IllegalAccessException, NoSuchFieldException {
        ...
        return items;
//        StoreItem[] items = (StoreItem[]) itemsField.get(itemList);
//        int size = (int) sizeField.get(itemList);
//
//        // Resize logic if necessary (double the array size)
//        if (size >= items.length) {
//            StoreItem[] larger = new StoreItem[items.length * 2];
...
//        }
    ...
    public void approachMasteryTestItemListremoveItem1() {
        ...
        try {
            ...
            // Set a timeout for the thread execution (e.g., 10 seconds)
            future.get(10000000, MILLISECONDS);
        } catch (TimeoutException e) {
            // Thread execution took longer than the specified timeout
            fail("Test timed out");
        } catch (Exception e) {
    ...
    void fullMasteryTestRefillInventory1() {
        ...
        try {
            Future<?> future = executor.submit(() -> {
                ...
                assertEquals(7, allItems.length, "getAllItems should return an array of size 2");
                assertTrue(allItems[4] instanceof Drink, "First element should be a Drink object");
                assertTrue(allItems[5] instanceof Drink, "Second element should be a Drink object");
                assertTrue(allItems[6] instanceof Drink, "Third element should be a Drink object");
                assertEquals(drink1, allItems[4], "First element should be drink1");
                assertEquals(drink2, allItems[5], "Second element should be drink2");
                assertEquals(drink3, allItems[6], "Third element should be drink3");
                assertEquals(7, storeItemsAfter.getSize(), "getSize should return 3");
                fullMasteryPassed++;
            });
            // Set a timeout for the thread execution (e.g., 10 seconds)
            future.get(250, MILLISECONDS);
    ...
    void fullMasteryTestRefillInventory5() {
        ...
        try {
            Future<?> future = executor.submit(() -> {
                ...
                ItemList storeItemsAfter = store.getItems();
                StoreItem[] allItems = storeItemsAfter.getAllItems();
                assertEquals(6, allItems.length, "getAllItems should return an array of size 7");
                assertTrue(allItems[0] instanceof Drink, "First element should be a Drink object");
                assertTrue(allItems[1] instanceof Drink, "Second element should be a Drink object");
                assertTrue(allItems[2] instanceof Drink, "Third element should be a Drink object");
                assertTrue(allItems[5] instanceof Drink, "Sixth element should be a Drink object");
                assertEquals(6, ((Drink) allItems[0]).getNumOfBottles(), "numBottles should be 6 for first drink in the store");
                assertEquals(4, ((Drink) allItems[1]).getNumOfBottles(), "numBottles should be 1 for second drink in the store");
                assertEquals(2, ((Drink) allItems[2]).getNumOfBottles(), "numBottles should be 2 for third drink in the store");
                assertEquals(3, ((Drink) allItems[5]).getNumOfBottles(), "numBottles should be 3 for fourth drink in the store");
                fullMasteryPassed++;
            });
            // Set a timeout for the thread execution (e.g., 10 seconds)
            future.get(250, MILLISECONDS);
